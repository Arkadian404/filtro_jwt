...
@Component({
  selector: 'app-bottled-coffee',
  templateUrl: './bottled-coffee.component.html',
...
export class BottledCoffeeComponent implements OnInit{
  ...
  getProducts(page?:number, sort?:string, flavor?:string, category?:string, brand?:string, origin?:string, vendor?:string){
    return this.productService.getBottledCoffeeListPaging(page, sort, flavor, category, brand, origin, vendor)
      .subscribe({
        next:(data)=>{
          console.log(data);
          this.page = data;
