...
@Component({
  ...
  templateUrl: './americas-coffee.component.html',
  styleUrls: ['../collection.component.scss']
})
export class AmericasCoffeeComponent implements OnInit{
  title = "Khu vực Châu Mỹ"
  isBrandClose = true;
  ...
  getProducts(page?:number, sort?:string, flavor?:string, category?:string, brand?:string, origin?:string, vendor?:string){
    return this.productService.getContinentCoffeeListPaging("Châu Mỹ" ,page, sort, flavor, category, brand, origin, vendor)
      .subscribe({
        next:(data)=>{
          console.log(data);
          this.page = data;
  ...
  getOrigins(){
    this.originService.getProductOriginContinentList("Châu Mỹ").subscribe({
      next:(data)=>{
        this.origins = data;
      },
      error:(err)=>{
